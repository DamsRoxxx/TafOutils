...
# if RTIBOOST_WORKAROUND(RTIBOOST_MSVC,>=1300) && defined(_MSC_EXTENSIONS)
        ...
        struct encode_counter
        {
            ...
            {
                RTIBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 257>::count));
            }
            __if_not_exists(the_counter<N + 256>)
            {
                __if_exists(the_counter<N + 64>)
                {
                    RTIBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 65>::count));
                }
                __if_not_exists(the_counter<N + 64>)
                {
                    __if_exists(the_counter<N + 16>)
                    {
                        RTIBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 17>::count));
                    }
                    __if_not_exists(the_counter<N + 16>)
                    {
                        __if_exists(the_counter<N + 4>)
                        {
                            RTIBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 5>::count));
                        }
                        __if_not_exists(the_counter<N + 4>)
                        {
                            __if_exists(the_counter<N>)
                            {
                                RTIBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 1>::count));
                            }
                            __if_not_exists(the_counter<N>)
                            {
                                RTIBOOST_STATIC_CONSTANT(unsigned,count=N);
...
# define RTIBOOST_TYPEOF_INDEX(T) (encode_counter<T>::count)
# define RTIBOOST_TYPEOF_NEXT_INDEX(next)
# else
...
# define RTIBOOST_TYPEOF_INDEX(T) (sizeof(*rtiboost::type_of::encode_index((rtiboost::type_of::encode_counter<1005>*)0)))
...
#elif RTIBOOST_WORKAROUND(RTIBOOST_MSVC,>=1400)
...
# if 0 //RTIBOOST_WORKAROUND(RTIBOOST_MSVC,==1310)
...
# define RTIBOOST_TYPEOF(expr) \
    rtiboost::type_of::msvc_typeid_wrapper<typeid(rtiboost::type_of::encode_start(expr))>::type

# define RTIBOOST_TYPEOF_TPL(expr) typename RTIBOOST_TYPEOF(expr)

# define RTIBOOST_TYPEOF_NESTED_TYPEDEF_TPL(name,expr) \
struct name {\
    enum {_typeof_register_value=sizeof(typeid(rtiboost::type_of::typeof_register_type<name>(expr)))};\
