...
#ifndef RTIBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED
#define RTIBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED

#include <rtiboost/type_traits/integral_constant.hpp>
#include <rtiboost/detail/workaround.hpp>

#if !defined(RTIBOOST_NO_CXX11_VARIADIC_TEMPLATES) && !defined(RTIBOOST_NO_CXX11_DECLTYPE) && !RTIBOOST_WORKAROUND(RTIBOOST_MSVC, < 1800) && !RTIBOOST_WORKAROUND(RTIBOOST_GCC_VERSION, < 40500)
...
namespace rtiboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(rtiboost::declval<TheArgs>()...))>
         static rtiboost::type_traits::yes_type test(int);
         template<typename, typename...>
         static rtiboost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(rtiboost::declval<Arg>()))>
         static rtiboost::type_traits::yes_type test1(int);
         template<typename, typename>
         static rtiboost::type_traits::no_type test1(...);

         template <typename T>
         static rtiboost::type_traits::yes_type ref_test(T);
         template <typename T>
         static rtiboost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(rtiboost::type_traits::yes_type)>{};
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(rtiboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(rtiboost::declval<Arg>())) == sizeof(rtiboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(rtiboost::declval<Arg>())) == sizeof(rtiboost::type_traits::yes_type)>{};
...
#endif // RTIBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED
