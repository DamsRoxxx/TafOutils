...
// Preprocessed version of "rtiboost/mpl/plus.hpp" header
...
template<> struct plus_impl< na,rtiboost_integral_c_tag >
{
    template< typename U1, typename U2 > struct apply
    {
        typedef apply type;
...
template<> struct plus_impl< rtiboost_integral_c_tag,na >
{
    template< typename U1, typename U2 > struct apply
    {
        typedef apply type;
...
{
    RTIBOOST_MPL_AUX_LAMBDA_SUPPORT(2, plus2, (N1, N2))

};

RTIBOOST_MPL_AUX_NA_SPEC2(2, 5, plus)

}}

namespace rtiboost { namespace mpl {
template<>
struct plus_impl< rtiboost_integral_c_tag,rtiboost_integral_c_tag >
{
