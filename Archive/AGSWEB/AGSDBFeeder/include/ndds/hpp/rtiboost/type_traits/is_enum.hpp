...
{
   RTIBOOST_STATIC_CONSTANT(bool, value = ::rtiboost::is_class<T>::value || ::rtiboost::is_union<T>::value);
};
...
struct is_class_or_union
{
# if RTIBOOST_WORKAROUND(__BORLANDC__, RTIBOOST_TESTED_AT(0x581))// we simply can't detect it this way.
...
#  if RTIBOOST_WORKAROUND(__MWERKS__, <= 0x3000) // no SFINAE
...
struct is_enum_helper<false>
{
    template <typename T> struct type
    {
       static const bool value = ::rtiboost::is_convertible<typename rtiboost::add_reference<T>::type, ::rtiboost::detail::int_convertible>::value;
    };
};
...
template <class T> struct is_enum : public integral_constant<bool, ::rtiboost::detail::is_enum_impl<T>::value> {};
...
template <class T> struct is_enum : public integral_constant<bool, RTIBOOST_IS_ENUM(T)> {};
