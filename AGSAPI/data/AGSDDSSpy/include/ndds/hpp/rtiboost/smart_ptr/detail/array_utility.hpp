 ...
 */
#ifndef RTIBOOST_SMART_PTR_DETAIL_ARRAY_UTILITY_HPP
#define RTIBOOST_SMART_PTR_DETAIL_ARRAY_UTILITY_HPP
...
namespace rtiboost {
    namespace detail {
        typedef rtiboost::true_type  ms_is_trivial;
        typedef rtiboost::false_type ms_no_trivial;
        ...
        inline void ms_destroy(T* memory, std::size_t size) {
            rtiboost::has_trivial_destructor<T> trivial;
...
#if !defined(RTIBOOST_NO_EXCEPTIONS)
            std::size_t i = 0;
            try {
                for (; i < size; i++) {
                    void* p1 = memory + i;
        ...
        inline void ms_init(T* memory, std::size_t size) {
            rtiboost::has_trivial_default_constructor<T> trivial;
